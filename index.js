...
function getAvailableDestinations() {
    const destinationavilable= destinations.find(dest=>dest.available===true);
    return destinationavilable.length;
}
...
function groupBookingsByStatus() {
    ...
    bookings.forEach(Element=>{
            ...
            const obj={
                confirmed: bookings.filter(Element=>Element.status=="confirmed") ,
                pending:  bookings.filter(Element=>Element.status=="pending"),
                cancelled:  bookings.filter(Element=>Element.status=="cancelled")
            }
        statustab.push(obj);
        return statustab;     
...
function findMostExpensiveBooking() {
    let max=bookings[0].totalPrice;
     for(let Element of bookings){
        if(Element.totalPrice>max){
            max=Element.totalPrice;
        }  
     }
...
function getBookingSummary() {
    ...
    bookings.forEach(Element=>{
        const elm={
            ...
            passengers:Element.passengers.length,
